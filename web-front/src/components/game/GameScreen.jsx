...
export default function GameScreen({onGoHome, onSetting, initialSave}) {
    ...
    // 자동 저장 + 다음 스토리 이동
    const handleAutoSaveAndNextStory = useCallback(
        async (lineIndex) => {
            try {
                await saveGame({
                    slot: 1,
                    storyId,
                    lineIndex,
                    heroineLikes: [], // 나중에 호감도 넣고 싶으면 여기 채우면 됨
                });
            } catch (e) {
                console.error(e);
                // 실패해도 진행은 되게 할지 여부는 취향
                alert("자동 저장에 실패했지만 다음 스토리로 진행합니다.");
            } finally {
                // 다음 스토리로 이동
                setStoryId((prev) => prev + 1); // or nextStoryId 등으로 변경 가능
                setPos(0);
            }
        },
        [saveGame, storyId, setStoryId, setPos]
    );
    ...
    const goToNextSequential = useCallback(() => {
        ...
        setPos((prev) => {
            const lastIndex = scriptLines.length - 1;

            // 이미 마지막 줄인데 또 진행을 누른 경우
            if (prev >= lastIndex) {
                // ▶ 여기서 자동 저장 + 다음 스토리로 이동
                handleAutoSaveAndNextStory(prev);
                return prev; // 현재 위치 유지 (곧 다음 스토리로 바뀔 거라서)
            }

            const next = prev + 1;
            return Math.min(next, lastIndex);
        });
    }, [scriptLines, handleAutoSaveAndNextStory]);
                        ...
                        <div className="bg-white p-4 rounded">
                            ...
                            <SaveMenu
                                ...
                                onLoad={(saveData) => {
                                    ...
                                    const { storyId: loadedStoryId, lineIndex: loadedIndex } = saveData;
